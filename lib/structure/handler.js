...
export default class isFiltered {
  ...
  async create() {
    ...
    const bot = (this.m.isGroup ? participants.find(u => Function.decodeJid(u.id) == this.sock.user.jid) : {}) || {}
    ...
    for (const name in plugins) {
      ...
      if(plugin.execute && typeof plugin.execute === "function") {
        ...
        if(plugin.command?.includes(this.m?.command) && this.m?.args[0] == "-i") {
          ...
          text += `• *File Plugin:* ../../${plugin.category}/${Object.keys(this.plugins).filter(v => v === plugin.command?.join(", "))[0]+'.js'}\n`;
  ...
  infinityIimit (isPrems, isMods, { isOwner }) {
    if(!isPrems || !isMods) db.user[this.m.sender].limit -= 1;
    if(!db.user[this.m.sender].limit < 0) return this.sock.sendMessage(this.m.chat, { text: `Maaf, Limit kamu sudah habis dan limit akan di reset setiap 24 jam, Informasi selengkapnya ketik ${m.prefix}slimit` }, { quoted: this.m })
    db.user[this.m.sender].premium = isOwner && this.m.fromMe;
